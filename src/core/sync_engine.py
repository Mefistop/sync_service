...
from config import LOCAL_PATH, TOKEN
...
class SyncEngine:
    ...
    def __init__(self, local_path, token, remote_folder_name: str = ""):
        ...
        self.cloud_api = CloudStorageApi(
            token=token, remote_folder_name=remote_folder_name
        )
    ...
    def delete_remote_files_not_on_local(self):
        ...
        if remote_files_not_on_local:
            ...
            logger.info(
                "Отсуствуют файлы из облака, не синхронизированные с локальной директорией."
            )
    ...
    def run_periodical(self, interval_seconds: int):
        ...
        while True:
            try:
                self.run_once()
                logger.info(
                    f"Синхронизация завершена. Ожидаю {interval_seconds} секунд до следующей итерации."
                )
                time.sleep(interval_seconds)
            except KeyboardInterrupt:
                logger.info(
                    f"Работа программы синхронизации файлов завершена. Спасибо, что выбрали наш продукт."
                )
                sys.exit(1)
