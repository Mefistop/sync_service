...
class CloudStorageApi:
    def __init__(self, token: str, remote_folder_name: str = ""):
        ...
        self.remote_folder_name = self.check_remote_folder(
    ...
    def get_info(self):
        ...
        if response.status_code >= 400:
            logger.error(
                f"Не удалось установить подключиться Яндекс Диску. Ошибка {response.json()['message']}."
            )
            return None
    ...
    def load(
        ...
        response_for_upload = self._make_request(
            method="get", endpoint="upload", params=params
        )

        if response_for_upload.status_code >= 400:
            logger.error(
                f"Файл '{local_file_path.split('/')[-1]}' не {['записан', 'перезаписан'][overwrite]}. "
                f"Ошибка: {response_for_upload.json()['message']}"
            )
            return None

        upload_info = response_for_upload.json()
        url = upload_info["href"]
        return self.load_file_to_remote(
    ...
    def load_file_to_remote(self, url, local_file_path: str, overwrite):
        ...
        try:
            ...
            if response.status_code not in (201, 202):
                logger.error(
                    f"Файл '{local_file_path.split('/')[-1]}' не {['записан', 'перезаписан'][overwrite]}. "
                    f"Ошибка: {response.json()['message']}"
                )
                return None
            logger.info(
                f"Файл '{local_file_path.split('/')[-1]}' успешно {['записан', 'перезаписан'][overwrite]}."
            )
            return True

        except requests.exceptions.ConnectionError:
            logger.error(
                f"Загрузка файла '{local_file_path.split('/')[-1]}' превысила лимит времени (15 секунд)."
            )
            return None

        except Exception as e:
            logger.error(
                f"Произошла ошибка при загрузке файла '{local_file_path.split('/')[-1]}': {str(e)}"
            )
            return None

    def reload(
        self, local_file_path: str, remote_file_name: str, overwrite: bool = True
    ):
    ...
    def is_exist_remote_folder(self, remote_folder_name):
        ...
        if response.status_code >= 400:
            message = response.json()["message"]
            logger.error(
                f"Директория '{remote_folder_name}' на облаке не существует. Ошибка: {message}"
            )
            return False
        return True
    ...
    def create_remote_folder(self, remote_folder_name):
        ...
        if response.status_code >= 400:
            message = response.json()["message"]
            logger.error(
                f"Директория '{remote_folder_name}' на облаке не создана. Ошибка: {message}."
            )
            return False
